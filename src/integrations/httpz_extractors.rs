...
pub trait TCtxFunc<TCtx, TMarker>: Clone + Send + Sync + 'static {
    fn exec<'req>(&self, req: httpz::Request) -> Result<TCtx, ExecError>
    where
        TCtx: Send + 'req;
}
...
{
    fn exec<'req>(&self, _req: httpz::Request) -> Result<TCtx, ExecError>
    where
        TCtx: Send + 'req,
    {
